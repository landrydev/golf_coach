...
| Critical journeys | Public, SIWC, instructor setup/return, consent grant/withdrawal, golfer authoring/share/revoke, living updates, external handoff, data requests/operator boundary, error/recovery | Exact-v9 signed-out `/`, `/app`, `/api/health`, and `/api/operations/health` requests each received the outer-policy `401`; the expected non-owner `/app.rsc` request returned `403`/`ok`. The local synthetic golfer fixture now uses the real capability-to-session exchange, but it is not a hosted or authorized real-account journey. Authenticated hosted and real-account journeys remain open. Deep readiness intentionally remains degraded until exact consent/operator configuration exists. |
